...
func LoggerHandler(h http.Handler, o *ioxer.IOXer) http.Handler {
	return NewLogger(h)
}
...
func mySingleHost(h http.Handler, o *ioxer.IOXer) http.Handler {
	return SingleHost(h, o, "erp.local:8443")
}

func SingleHost(handler http.Handler, o *ioxer.IOXer, allowedHost string) http.Handler {
...
func srvHome(h http.Handler, o *ioxer.IOXer) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		log.Printf("Inside Home")
		o.PutData("ok", "okay la")
		o.PutData("ok", "okay la"+r.FormValue("act"))
	})
}

func main() {
	chain := alice.New(alice.IOXerEchoHandler, LoggerHandler, mySingleHost, srvHome).Run(nil)
